...
public class LotteryGame {
    ...
    private RandomCollection<LotteryEntry> getBoughtTickets() {
    ...
    public double winningAmount() {
        ...
        amount = ticketsize * Utils.formatAmount(lConfig.getCost(), lConfig.isUseEconomy());
        ...
        amount = Utils.formatAmount(amount, lConfig.isUseEconomy());
    ...
    private double taxAmount() {
        ...
        final RandomCollection<LotteryEntry> players = getBoughtTickets();
        amount = players.size() * Utils.formatAmount(lConfig.getCost(), lConfig.isUseEconomy());
    ...
    private void addToClaimList(final UUID player, final int winningAmount, final int winningMaterial) {
    ...
    private void addToWinnerList(final String playerName, final Double winningAmount, final int winningMaterial) {
    ...
    public boolean getWinner() {
        ...
        if (players.isEmpty()) {
            ...
            if (lConfig.isUseEconomy()) {
                ...
                // Announce the winner:
                broadcastMessage("WinnerCongrat", winner.getName(), Utils.formatCost(amount, lConfig), ticketsBought, lConfig.getPlural("ticket", ticketsBought));
                ...
                final int matAmount = (int) Utils.formatAmount(amount, lConfig.isUseEconomy());
            ...
            int material = lConfig.isUseEconomy() ? -1 : lConfig.getMaterial();
    ...
    private void clearAfterGettingWinner() {

        // extra money in pot added by admins and mods?
        // Should this be removed?
        if (lConfig.isClearExtraInPot()) {
    ...
    private String formatCustomMessageLive(final String message, final Object... args) throws Exception {
        ...
        // %cost% = cost
        outMessage = outMessage.replaceAll("%cost%", Matcher.quoteReplacement(Utils.formatCost(lConfig.getCost(), lConfig)));

        // %pot%
        outMessage = outMessage.replaceAll("%pot%", Matcher.quoteReplacement(Utils.formatCost(winningAmount(), lConfig)));
